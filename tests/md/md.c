...
#define ATOMCOUNT 72
#define STEPCOUNT 10
#define CUBELENGTH 10
#define INVMASS 0.5
#define DT 0.02
#define BOLTZMAN 0.001987191
#define RAND_MAX 9948000
//#define VERBOSE 1
...
doMD(int atomCount, int stepCount)
{
    ...
    while (stepIndex++ < stepCount)
    {
        ...
        if(world_rank == 0){
            ...
            printf("%d\n\n",atomCount);
            for(q = 0; q < atomCount; q++){
                //printf("pos: %f\n",pos[q]);
                PRINT(" C %8.3f %8.3f %8.3f\n", pos[q * 3], pos[q * 3 + 1], pos[q * 3 + 2]);
            }
        }
        else{
        ...
        if (!world_rank)
        {
            ...
            for (i = world_rank * localAtomCount; i != world_rank * localAtomCount + localAtomCount; ++i)
            {
                ...
                temp = floatMod(vel[i * 3] * DT, CUBELENGTH);
                //if (!isnan(temp))
                    pos[i * 3] += temp;

                vel[i * 3 + 1] += forceSum[i * 3 + 1] * DT * INVMASS;
                temp = floatMod(vel[i * 3 + 1] * DT, CUBELENGTH);
                //if (!isnan(temp))
                    pos[i * 3 + 1] += temp;

                vel[i * 3 + 2] += forceSum[i * 3 + 2] * DT * INVMASS;
                temp = floatMod(vel[i * 3 + 2] * DT, CUBELENGTH);
                //if (!isnan(temp))
                    pos[i * 3 + 2] += temp;
            }
        }
